...
namespace Brightside\Vimeovideo\Preview;
 ...
 */
class VimeovideoPreviewRenderer extends StandardContentPreviewRenderer
{
    public function renderPageModulePreviewContent(GridColumnItem $item): string
    {
        ...
        $youtubeObjects = BackendUtility::resolveFileReferences('tt_content', 'tx_vimeovideo_assets', $row);
        $content .= '<div class="vimeovideo-container">';
        foreach ($youtubeObjects as $video) {
            ...
            if (!$video->getProperty('hidden')){
                $content .= '<div class="vimeovideo-item">';
            if ($extensionConfiguration['vimeovideoEnableBePlayer']) {
                $content .= '<div class="vimeovideo-wrapper"><iframe src="https://player.vimeo.com/video/';
                $content .= $code;
                ...
                $content .= '<ul>';
                if ($video->getProperty('tx_vimeovideo_mute')){
                ...
                if ($video->getProperty('tx_vimeovideo_endtime')){
                    $content .= '<li>' . $this->linkEditContent('End: ' . $video->getProperty('tx_vimeovideo_endtime') , $row) . '</li>';
                }
                $content .= '</ul></div>';
            }
        }
        ...
        if (
            ...
            if ($row['tx_vimeovideo_colcount']) {
                $content .= '<br /><b>Layout:</b> columns:' . $row['tx_vimeovideo_colcount'];
            }
